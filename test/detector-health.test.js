...
test("a missing pong transitions to disconnected, while a later pong can recover", () => {
  ...
  assert.equal(health.accept(state, validHealth).detectorVersion, "1.1.4");
  assert.equal(state.status, "connected");
});
...
test("malformed health payloads are rejected without changing connection state", () => {
  ...
  for (const payload of [
    ...
    { ...validHealth, telemetry: null },
    {
      ...validHealth,
      telemetry: { ...validHealth.telemetry, observed: -1 }
    }
  ]) {
...
test("page-controlled health getters cannot break the isolated listener", () => {
  const hostile = {};
  Object.defineProperty(hostile, "detectorVersion", {
    get() {
      throw new Error("page getter");
    }
  });
  assert.equal(health.normalizeHealth(hostile), null);
});
